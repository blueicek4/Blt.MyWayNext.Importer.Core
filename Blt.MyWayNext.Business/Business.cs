    ...
    public static class Business
    {
        ...
        public static async Task<MyWayApiResponse> ImportAnagraficaTemporanea(NameValueCollection form, string name)
        {
            ...
            try
            {
                ...
                var resTemporanea = await client.AnagraficheIbridePostAsync(nuovoCliente);
            ...
            try
            {
                ...
                if (mapAnagraficaTemporanea.Count > 0)
                {
                    ...
                    else
                    {
                        ...
                        log.Debug($"Invio richiesta di creazione anagrafica temporanea");
                        var resIbride = await client.AnagraficheIbridePostAsync(ObjAnagraficaIbrida);
                    ...
                    if (mapAggiornaIniziativa.Count > 0)
                    {
                        ...
                        if (resp.Code == "STD_OK")
                        {
                            ...
                            if (mapAttivitaCommerciale.Count > 0)
                            {
                                ...
                                RequestAttivita ReqAttivita = new RequestAttivita();
                                if (ObjAnagraficaIbrida.Temporanea ?? false)
                                {
                                ...
                                var ObjAttivitaContattoSalvata = await client.CommercialiAttivitaPostAsync(true, true, true, true, ObjAttivitaSvolta.Data);
                                ...
                                if (ObjAttivitaContattoSalvata.Code == "STD_OK")
                                {
                                    ...
                                    var ObjAttivitaSalvata = await client.CommercialiAttivitaPutAsync(false, true, true, true, objAttivitaDaFare.Data);
        ...
        public static async Task<MyWayApiResponse> GetAttivitaXIniziativa(RequestAttivitaByIniziativa iniziativa)
        {
            ...
            try
            {
                ...
                var funnel = await client.CommercialiTrattativeFunnelIDGetAsync(ObjIniziativa.Data.FunnelId.Value, false);
                var fasi = client.CommercialiTrattativeFasiListaGetAsync().GetAwaiter().GetResult().Data.ToDictionary(f => f.Id, f => f.Nome);
                var esiti = client.CommercialiTrattativeEsitiListaGetAsync().GetAwaiter().GetResult().Data.ToDictionary(f => f.Id, f => f.Nome); ;
                var stati = client.CommercialiTrattativeStatiListaGetAsync(false).GetAwaiter().GetResult().Data.ToDictionary(f => f.Id, f => f.Nome); ;
                ...
                if (ObjListAttivita.Code == "STD_OK")
                {
                    ...
                    else
                    {
                        iniziativaResponse.ElencoAttivita = ObjListAttivita.Data.Select(a => new AttivitaCommerciale
                        {
                            ...
                            FaseFunnel = fasi[a.FunnelFaseId],
                            StatoFunnel = stati[a.FunnelStatoId],
                            Esito = esiti[a.FunnelEsitoId],
                            Chiusa = a.Chiusa,
        ...
        public static async Task<MyWayApiResponse> GetAttivitaxPeriodo(GetRange range)
        {
            ...
            try
            {
                ...
                var aaa = new ViewPropertiesOfAttivitaViewConditionAndEntitiesAnd0AndCultureneutral();
                aaa.Condition.DataInizio = range.Start;
                aaa.Condition.DataFine = range.End;

                var body = new SchedulerCondition();
                body.LoadCommerciali = true;
                body.LoadService = false;

                body.Commerciali.StartDate = range.Start;
                body.Commerciali.EndDate = range.End;
                body.Commerciali.RisorseCod = new string[] { range.Agente }; //new <string() { range.Agente };
                ...
                if (ObjListAttivita.Code == "STD_OK")
                {
                    ...
                    else
                    {
                        ...
                        foreach (var att in AttivitaCommerciali)
                        {
                        ...
                        response.Success = true;
                        response.ErrorMessage = $"Attività commerciali esistenti | Rilevate {AttivitaCommerciali.Count} attività presenti";
                    }
                }
                else
                {
        ...
        public static async Task<MyWayApiResponse> AggiornaAttivitaCommerciale(AggiornaAttivitaCommerciale aggiornaAttivita)
        {
            ...
            try
            {
                ...
                var funnel = await client.CommercialiTrattativeFunnelIDGetAsync(Convert.ToInt32(aggiornaAttivita.Funnel), false);
                var fasi = client.CommercialiTrattativeFasiListaGetAsync().GetAwaiter().GetResult().Data.ToDictionary(f=> f.Id, f => f.Nome);
                var esiti = client.CommercialiTrattativeEsitiListaGetAsync().GetAwaiter().GetResult().Data.ToDictionary(f => f.Id, f => f.Nome); ;
                var stati = client.CommercialiTrattativeStatiListaGetAsync(false).GetAwaiter().GetResult().Data.ToDictionary(f => f.Id, f => f.Nome); ;
                ...
                if(objAttivitaDaFare.Code == "STD_OK")
                {
                    ...
                    var esito = esiti.First(e=>e.Value == aggiornaAttivita.attivitaSvolta.Esito).Key;
                    attivita.AttivitaSvolta = attivita.AttivitaSvoltaText = aggiornaAttivita.attivitaSvolta.AttivitaSvolta;

                    attivita.FunnelEsitoId = esito;
                    
                    var objAttivitaAggiornata = await client.CommercialiAttivitaPutAsync(true, true, true, true, attivita);
                    ...
                    if (ObjAttivita.Code == "STD_OK")
                    {
                        ...
                        ObjAttivita.Data.FunnelFaseId = fase;
                        ObjAttivita.Data.FunnelStatoId = stato;
                        var ObjNuovaAttivita = await client.CommercialiAttivitaPutAsync(false, true, true, true, ObjAttivita.Data);
        ...
        public static async Task<MyWayApiResponse> AggiornaIniziativaCommerciale(AssegnaIniziativa iniziativaAggiornata)
        {
            ...
            try
            {
                ...
                if (objIniziativa.Code == "STD_OK")
                {
                    ...
                    var res = await client.CommercialiIniziativaPutAsync(iniziativa);
                    ...
                    foreach (var att in res.Data.Attivita.Where(a => a.Stato.DisplayValue == "Da Svolgere"))
                    {
                        ...
                        {                            
                            ...
                            var resAttivita = await client.CommercialiAttivitaPutAsync(false, false, false, false, attivita.Data);
        ...
        public static async Task<MyWayStatiResponse> GetStatiTrattiva()
        {
            ...
            try
            {
                ...
                var objStatiResp = await client.CommercialiTrattativeStatiListaGetAsync(false);
        ...
        public static async Task<MyWayApiResponse> SetTrattativaCommerciale(MyWayObjTrattativa trattativa)
        {
            ...
            try
            {
                ...
                var objTrattiveResp = await client.CommercialiTrattativePutAsync(objTrattativeUpdate);
        ...
        public static async Task<MyWayApiResponse> PutTrattativaCommerciale(MyWayObjTrattativa trattativa)
        {
            ...
            try
            {
                ...
                var objNuovaTrattativaResp = await client.CommercialiTrattativePostAsync(false, objNuovaTrattativa.Data);
        ...
        public static async Task<MyWayApiResponse> PutAnagraficaAsync(AnagraficaIbrida anagrafica)
        {
            ...
            try
            {
                ...
                var anagraficaAggiornata = await client.AnagraficheIbridePutAsync(anagrafica);
